...
export default function ReactChain(vdom) {
    return processVdom(vdom).then(vdom => {
        if (isVDom(vdom)) {
            var uuid = UUID();
    ...
    if (isVDom(vdom)) {
        ...
        return new Promise(function(resolve, reject) {
            ...
            if (queue.length == 0) {
                ...
                queue.map(item => {
                    Promise.resolve(processVdom(item.obj[item.key])).then(function(child) {
            ...
            if (vdom[1] == 'include') {
                ...
                return Promise.resolve(typeof comp == 'function' ? comp(props, children) : comp).then(function(comp) {
                    return processVdom(comp);
                });
            }
            return vdom;
        }).catch(e => {
    ...
    if (Array.isArray(vdom)) {
        if (vdom[0] == React) {
            ...
            if (typeof vdom[1] == 'function') {
                vdom[1] = {$class: vdom[1].browserPath};
            }
...
for (var k in ReactMaterialize) {
    if (typeof ReactMaterialize[k] == 'function' && ReactMaterialize[k].prototype.render) {
        ReactMaterialize[k].browserPath = ['ReactMaterialize', k];
    }
}
